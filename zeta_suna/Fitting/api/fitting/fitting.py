...
class Fitting():
    ...
    def __init__(self, kafka_infos):
        ...
        self.output_dir = self.kafka_infos['outputDir']
    ...
    def run(self):
        ...
        try:
            ...
            row_template = {
                '序号': self.count,
                '文件名': 'NA',
                'Pattern Current Row': 'NA',
                'Pattern Current Column': 'NA',
            ...
            if self.file_type == 'xyz':
                ...
                if len(str_split) == 3:
                    ...
                    row_template['Pattern Current Row'] = int(str_split[1])  # 转换为整型
                    row_template['Pattern Current Column'] = int(str_split[2].split('.')[0])  # 转换为整型
            elif self.file_type == 'csv':
                row, col = get_row_col_info(self.file_path)
                row_template['文件名'] = os.path.basename(self.file_path) 
                row_template['Pattern Current Row'] = row  
                row_template['Pattern Current Column'] = col  
        ...
        maxroc, minroc, maxconic, minconic) = self.fitrocrms4_ho1(
                ...
                row_template[f'RMS_{axis}'] = minrmsdelta
    ...
    def write_excel_fitting(self, data):
        # 固定表头
        fixed_header = [
            '序号', '文件名', 'Pattern Current Row', 'Pattern Current Column',
            '起始ROC', '终止ROC', 'ROC步进',
        ...
        output_path = os.path.join(self.output_dir, f"{self.file_type}_summary.xls")
