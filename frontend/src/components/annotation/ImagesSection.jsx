      ...
      if (data.success) {
        ...
        if (!currentSessionImages.has(imageId)) {
          ...
          if (!existingFilenames.has(data.filename)) {
            ...
            currentSessionImages.set(imageId, imageData);
            currentBatchImages.push(imageData);
            currentBatchAddedCount++;
            console.log(`[processFile] 成功添加图片: ${data.filename}, 尺寸: ${data.width}x${data.height}, 大小: ${data.size}`);
          } else {
...
// 计算objectFit为cover时的实际显示区域
  ...
  const tableData = paginatedImages.map(img => {
    ...
    if (!img.height && !img.width && img.url) {
      // 异步获取尺寸（不影响渲染）
      fetchImageDimensions(img.url).then(dimensions => {
        if (dimensions) {
          ctx.setAllImages(prev => prev.map(prevImg => 
            prevImg.id === img.id ? { ...prevImg, width: dimensions.width, height: dimensions.height } : prevImg
          ));
        }
      }).catch(error => {
        console.log(`无法获取图片 ${img.filename} 的尺寸:`, error);
      });
    }
  ...
  const handleDownloadImage = async (img) => {
    if (!img.url) {
      console.warn('图片URL不存在，无法下载:', img);
      return;
    }
  ...
  const handleClearSelection = async () => {
    ...
    Modal.confirm({
      title: '确认删除',
      content: `确定要删除选中的 ${selectedImages.size} 张图片吗？此操作将同时删除图片文件和对应的标注文件。`,
      onOk: async () => {
        // 获取选中的图片信息
        const selectedImageData = ctx.allImages.filter(img => selectedImages.has(img.id));
        const filenames = selectedImageData.map(img => img.filename);
        ...
        // 后端删除
        try {
          console.log('[批量删除] 准备发送删除请求:', {
            filenames: filenames,
            dataset_id: ctx.currentDataset,
          ...
          console.log('[批量删除] 响应状态:', response.status);
          const result = await response.json();
          console.log('[批量删除] 响应结果:', result);
          if (result.success) {
            appMessage.success(result.message || `成功删除 ${selectedImages.size} 张图片`);
            ...
            try {
              ...
              if (datasetsData.success && datasetsData.data) {
                console.log('[批量删除] 重新加载数据集列表:', datasetsData.data);
                // 直接更新数据集列表
                ctx.setDatasets(datasetsData.data);
                // 触发数据集更新事件，通知Layout刷新
                window.dispatchEvent(new CustomEvent('datasets-updated', { detail: datasetsData.data }));
              }
            } catch (reloadError) {
              console.error('[批量删除] 重新加载数据集列表失败:', reloadError);
            }
          } else {
            appMessage.error(result.message || '删除失败');
          }
        } catch (error) {
          console.error('删除失败:', error);
          appMessage.error(`删除失败: ${error.message}`);
        }
  ...
  // 一键划分数据集
  const handleAutoSplitDataset = async () => {
    Modal.confirm({
      title: '一键划分数据集',
      content: (
        <div>
          <p>将自动按照 8:1:1 的比例划分数据集为 Train:Val:Test</p>
          <p style={{ color: '#fa8c16', fontSize: 12 }}>
            注意：此操作将重新分配所有图片到Train/Val/Test，现有的划分将被覆盖
          </p>
        </div>
      ),
      onOk: async () => {
        try {
          appMessage.loading('正在划分数据集...', 0);
          
          const response = await fetch('http://localhost:8000/api/train/split-dataset-by-ratio', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
              dataset_id: ctx.currentDataset,
          ...
          if (result.success) {
            appMessage.destroy();
            appMessage.success(result.message || '数据集划分完成');
            
            // 重新加载图片列表
            await ctx.switchDataset(ctx.currentDataset);
            ...
            appMessage.destroy();
            appMessage.error(result.message || '划分失败');
          }
        } catch (error) {
          appMessage.destroy();
          console.error('划分数据集失败:', error);
          appMessage.error(`划分失败: ${error.message}`);
        }
      }
    });
  };
    ...
    <div ref={containerRef} style={{ 
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, background: '#fff', padding: '8px 12px', borderRadius: 8, border: '1px solid #e8e8e8' }}>
        ...
        <Button
          ...
          size="small"
          type="default"
          onClick={handleAutoSplitDataset}
        >
          一键划分
        </Button>

        <Badge count={ctx.allImages.length} showZero style={{ backgroundColor: '#1890ff' }}>
          <Button type={ctx.currentSplit === 'all' ? 'primary' : 'default'} onClick={() => ctx.setCurrentSplit('all')} size="small">All</Button>
        </Badge>
        <Badge count={ctx.allImages.filter(i => i.split === 'train').length} showZero style={{ backgroundColor: '#52c41a' }}>
          <Button type={ctx.currentSplit === 'train' ? 'primary' : 'default'} onClick={() => ctx.setCurrentSplit('train')} size="small">Train</Button>
        </Badge>
        <Badge count={ctx.allImages.filter(i => i.split === 'val').length} showZero style={{ backgroundColor: '#fa8c16' }}>
          <Button type={ctx.currentSplit === 'val' ? 'primary' : 'default'} onClick={() => ctx.setCurrentSplit('val')} size="small">Val</Button>
        </Badge>
        <Badge count={ctx.allImages.filter(i => i.split === 'test').length} showZero style={{ backgroundColor: '#f5222d' }}>
          <Button type={ctx.currentSplit === 'test' ? 'primary' : 'default'} onClick={() => ctx.setCurrentSplit('test')} size="small">Test</Button>
        </Badge>
      </div>
            ...
            >
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.parentElement.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#999;font-size:12px;">图片加载失败</div>';
                }}
