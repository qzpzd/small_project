...
import { Card, Form, Input, Button, Upload, Slider, Dropdown, Tag, Tooltip, Modal, ColorPicker, Select, Spin } from 'antd';
import { 
  ...
  SyncOutlined, UndoOutlined, RedoOutlined, BookOutlined
...
const AnnotationView = () => {
  ...
  // 检查鼠标是否在矩形框的控制点上
  const checkHandleHover = (e, box) => {
    if (!svgRef.current || !box) return null;
    ...
    for (const [handle, pos] of Object.entries(handles)) {
      const distance = Math.sqrt(Math.pow(svgPoint.x - pos.x, 2) + Math.pow(svgPoint.y - pos.y, 2));
  ...
  const handleMouseEnter = (e) => {
    if (selectedBox) {
      const handle = checkHandleHover(e, selectedBox);
      if (handle) {
  ...
  const handleMouseMoveWithCursor = (e) => {
    // 如果有选中的矩形框，检查鼠标是否在控制点上
    if (selectedBox && !draggingBox && !draggingHandle) {
      const handle = checkHandleHover(e, selectedBox);
      setHoveredHandle(handle);
    }
  };
  ...
  const [hoveredHandle, setHoveredHandle] = useState(null); // 跟踪鼠标悬停的控制点
...
// 监听图片尺寸变化
  ...
  const updateMouse = useCallback((e) => {
    ...
    // 检查鼠标是否在图像实际显示区域内（考虑object-fit: contain的空白区域）
...
useEffect(() => {
    ...
    // 直接处理鼠标移动，不使用requestAnimationFrame减少延迟
  ...
  }, [ctx.selectedImage, naturalImageSize]);
...
// 处理鼠标移动事件
  const handleMouseMove = (e) => {
    ...
    // 使用SVG的坐标转换方法，考虑viewBox和preserveAspectRatio
    const svg = svgRef.current;
    const point = svg.createSVGPoint();
    point.x = e.clientX;
    point.y = e.clientY;
  ...
  const handleMouseUp = (e) => {
    ...
    // 使用SVG坐标转换方法，与handleMouseMove保持一致
    const point = svg.createSVGPoint();
    point.x = e.clientX;
    point.y = e.clientY;
    ...
    // 限制坐标在图像范围内
    const currentX = Math.max(0, Math.min(svgPoint.x, naturalImageSize.width));
    const currentY = Math.max(0, Math.min(svgPoint.y, naturalImageSize.height));
  ...
  useEffect(() => {
    ...
    const move = (e) => {
      ...
      // 检测是否真正移动（移动距离超过3像素才算拖拽）
      const deltaX = Math.abs(e.clientX - dragOffset.startX);
      const deltaY = Math.abs(e.clientY - dragOffset.startY);
      ...
      // 限制坐标在图像范围内
      const originalX = Math.max(0, Math.min(svgPoint.x, naturalImageSize.width));
      const originalY = Math.max(0, Math.min(svgPoint.y, naturalImageSize.height));
      
      const b = { ...draggingBox };
      b.x = originalX - dragOffset.x;
      b.y = originalY - dragOffset.y;
      
      // 确保矩形框在图像范围内
      b.x = Math.max(0, Math.min(b.x, naturalImageSize.width - b.w));
      b.y = Math.max(0, Math.min(b.y, naturalImageSize.height - b.h));
    ...
    window.addEventListener("mousemove", move);
    return () => window.removeEventListener("mousemove", move);
  }, [draggingBox, boxes, dragOffset, naturalImageSize, ctx.selectedImage, hasMoved]);
  ...
  useEffect(() => {
    const key = (e) => {
      ...
      if (e.key === "Delete" && selectedBox) {
        ...
        // 如果所有矩形框都被删除，删除对应的标签文件
        if (updated.length === 0 && ctx.selectedImage) {
          console.log('[删除矩形框] 所有矩形框已删除，删除标签文件');
          ctx.deleteJSONFile(ctx.selectedImage.id);
          ctx.deleteYOLOFile(ctx.selectedImage.id);
        }
      }
        ...
        <div style={{
          ...
          <svg
            ...
            style={{
              ...
              height: '100%',
              zIndex: 2,
              cursor: hoveredHandle ? getCursorForHandle(hoveredHandle) : 'crosshair'
            }}
            width="100%"
            height="100%"
          ...
          >
            ...
            {imgLoaded && isDrawing && startPoint && currentRect && (
              <rect
                x={currentRect.x}
                y={currentRect.y}
                  ...
                  <Tooltip title="删除该标签">
                    <Button
                      ...
                      onClick={(e) => {
                        ...
                        // 使用新的label列表更新data.yaml文件
                        ctx.updateDataYamlWithLabels(updatedLabels);
                      }}
                    />
                  </Tooltip>
                </div>
